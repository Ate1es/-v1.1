...
################################################################################################
# - 주요 정보 통신 기반 시설 | 파일 및 디렉터리 관리 | U-13 SUID, SGID, Sticky bit 권한 설정
################################################################################################
echo "[ U-13 ] : Check"
echo "====================== [U-13 SUID, SGID, Sticky bit 권한 설정 START]" >> $RESULT_FILE 2>&1
echo "" >> $RESULT_FILE 2>&1
...
echo "" >> $RESULT_FILE 2>&1
echo "====================== [U-13 SUID, SGID, Sticky bit 권한 설정 END]" >> $RESULT_FILE 2>&1
echo "" >> $RESULT_FILE 2>&1

################################################################################################
# - 주요 정보 통신 기반 시설 | 서비스 관리 | U-20 Anonymous FTP 비활성화
################################################################################################
echo "[ U-20 ] : Check"
...
if [ "$get_ps" != "" ] || [ "$get_service" != "" ] || [ "$get_port" != "" ]; then
	echo "FTP Configuration Check" >> $RESULT_FILE 2>&1
	if [ $vsftp_flag -eq 0 ]; then
		...
		if [ -f "/etc/passwd" ]; then
			...
			if [ "$user_chk" != "" ]; then
				FTP=0
				echo "$user_chk" >> $RESULT_FILE 2>&1
			else
				echo "Not Found FTP User" >> $RESULT_FILE 2>&1
			fi
		else
			echo "Not Found /etc/passwd File" >> $RESULT_FILE 2>&1
		fi
	else
		echo "vsFTP Configuration File" >> $RESULT_FILE 2>&1
		if [ -f "/etc/vsftpd/vsftpd.conf" ]; then
			echo "- File Name : /etc/vsftpd/vsftpd.conf" >> $RESULT_FILE 2>&1
			conf_chk=`cat /etc/vsftpd/vsftpd.conf | grep -v '^#' | grep 'anonymous_enable'`
		elif [ -f "/etc/vsftpd.conf" ]; then
			echo "- File Name : /etc/vsftpd.conf" >> $RESULT_FILE 2>&1
			conf_chk=`cat /etc/vsftpd.conf | grep -v '^#' | grep 'anonymous_enable'`
		else
			echo "- Configuration File Not Found" >> $RESULT_FILE 2>&1
		fi
		if [ "$conf_chk" != "" ]; then
			...
			echo "- Anoymous Enable Configuration Not Found" >> $RESULT_FILE 2>&1
		fi
	fi
fi
...
################################################################################################
# - 주요 정보 통신 기반 시설 | 서비스 관리 | U-24 NFS 비활성화
################################################################################################
echo "[ U-24 ] : Check"
echo "====================== [U-24 NFS 비활성화 START]" >> $RESULT_FILE 2>&1
echo "" >> $RESULT_FILE 2>&1
...
if [ "$get_rpcinfo" != "" ]; then
	...
	echo "- Not Found NFS" >> $RESULT_FILE 2>&1
fi
...
echo "" >> $RESULT_FILE 2>&1
echo "====================== [U-24 NFS 비활성화 END]" >> $RESULT_FILE 2>&1
echo "" >> $RESULT_FILE 2>&1
